...
function EtSubmit({ outHoverColor, inHoverColor, placeHolder }) {
    const [onHover, setOnOhover] = React.useState(false);
    ...
    return (jsxRuntime.jsx("div", { className: `w-[100%] font-bold`, children: jsxRuntime.jsx("button", { style: onHover ? { backgroundColor: inHoverColor.button, color: inHoverColor.text } : { backgroundColor: outHoverColor.button, color: outHoverColor.text }, onMouseEnter: () => setOnOhover(true), onMouseLeave: () => setOnOhover(false), type: "submit", className: `${buttonStyle} ${buttonStyleOnHover} mx-auto transition-all duration-500`, children: placeHolder }) }));
}
...
function EtInput({ inputType, placeHolder, insideInputColor, linesColor, minLength, maxLength }) {
    ...
    */
    ...
    return (jsxRuntime.jsxs("div", { className: `w-[100%] font-semibold flex flex-col`, children: [jsxRuntime.jsx("span", { style: !isFocused ? { backgroundColor: insideInputColor, color: linesColor } : { background: "none", color: linesColor }, className: `z-10 w-fit px-3 rounded-md ${isFocused ? posFocusTextStyle : preFocusTextStyle} transition-all duration-500`, children: placeHolder }), jsxRuntime.jsxs("div", { className: "relative", children: [jsxRuntime.jsx("input", { maxLength: maxLength, minLength: minLength, style: !isFocused ? { background: 'inherit', borderColor: linesColor, color: linesColor } : { backgroundColor: linesColor, color: insideInputColor }, className: `border-solid border-[1px] px-[1rem] outline-none rounded-md h-[2.9rem] w-[100%] ${isFocused ? posFocusInput : preFocusInput} transition-all duration-[1000ms]`, type: inputType == "password" && showPassword ? 'text' : inputType, name: "", id: "", onFocus: () => setIsFocused(true), onBlur: () => setIsFocused(false) }), inputType == "password"
                        ?
                            jsxRuntime.jsxs("div", { children: [jsxRuntime.jsx(LiaEye, { onClick: () => setShowPassword(true), style: !isFocused ? { color: linesColor } : { color: insideInputColor }, className: `select-none hover:cursor-pointer text-xl absolute right-4 top-3.5 ${showPassword ? 'hidden' : null}` }), jsxRuntime.jsx(LiaEyeSlash, { onClick: () => setShowPassword(false), style: !isFocused ? { color: linesColor } : { color: insideInputColor }, className: `select-none hover:cursor-pointer text-xl absolute right-4 top-3.5 ${!showPassword ? 'hidden' : null}` })] })
                        :
                            null] })] }));
}

exports.EtInput = EtInput;
exports.EtSubmit = EtSubmit;
